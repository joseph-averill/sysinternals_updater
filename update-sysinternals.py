...
def main():
	...
	if os.path.isfile(configfile):
		...
		#UpdatedDate = datetime.fromtimestamp(mktime(config['DEFAULT']['LastUpdated']))
		UpdatedDate = datetime(int(config['DEFAULT']['UpdateYear']),int(config['DEFAULT']['UpdateMonth']),int(config['DEFAULT']['UpdateDay']),0,0,0) 
		print('sysinternals last updated on ',UpdatedDate)
	else:
		print("file does not exist")
		#create the file
		InstallPath=input('It looks like you haven''t run this program before. Where would you like to save your sysinternals programs? ')
